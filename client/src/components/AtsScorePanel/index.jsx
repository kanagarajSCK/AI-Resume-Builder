...
function AtsScorePanel() {
  ...
  const handleAnalyze = async (options = {}) => {
    ...
    const nextJobDescription = options.jobDescription ?? jobDescription;
    setIsAnalyzing(true);
    try {
      updateField('jobDescription', nextJobDescription);
      const result = await getAtsScore(resume._id, nextJobDescription);
      updateAtsScore(result);
      toast.success(options.successMessage || 'ATS analysis complete!');
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const shouldAutoAnalyze = params.get('autoAnalyze') === '1';

    if (!shouldAutoAnalyze || hasAutoAnalyzed.current || !resume._id || isAnalyzing || atsScore?.overall > 0) {
      return;
    }
    ...
    handleAnalyze({
      ...
      successMessage: 'ATS analysis complete!',
    });
  }, [location.search, resume._id, resume.jobDescription, atsScore?.overall, isAnalyzing]);
