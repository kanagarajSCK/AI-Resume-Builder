...
function HomePage() {
  ...
  const [uploadMode, setUploadMode] = useState('improve');
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      navigate(
        uploadMode === 'ats'
          ? `/builder/${resume._id}?tab=ats&autoAnalyze=1`
          : `/builder/${resume._id}`
      );
    } catch (error) {
        ...
        {showUpload && (
          <div className="modal-overlay">
            <div className="modal-content" style={{ maxWidth: '480px' }}>
              <h2 className="heading-md mb-md">
                {uploadMode === 'ats' ? 'Upload Resume for ATS Analysis' : 'Upload Existing Resume'}
              </h2>
              <p className="text-muted mb-lg">
                {uploadMode === 'ats'
                  ? 'Upload a PDF and we will parse it, open the ATS tab, and generate a score automatically.'
                  : 'Upload a PDF and our AI will parse it into editable sections.'}
              </p>
              ...
              {!isUploading ? (
                <>
                  ...
                  <div className="flex-row gap-sm" style={{ marginTop: '24px' }}>
                    ...
                    <button onClick={handleUpload} disabled={!uploadFile} className={`btn btn-primary-gradient ${!uploadFile ? 'btn-disabled' : ''}`} style={{ flex: 1 }}>
                      {uploadMode === 'ats' ? 'Analyze ATS Score' : 'Parse & Import'}
                    </button>
                  </div>
                </>
              ) : (
                <div className="flex-col items-center gap-md" style={{ padding: '32px 0' }}>
                  ...
                  <p className="text-muted">
                    {uploadMode === 'ats' ? 'Preparing ATS analysis...' : 'Parsing your resume with AI...'}
                  </p>
